...
def MotionPolicy(list_pos0, list_pos, obstacles, goal):
    list_vel = (list_pos - list_pos0)  # might have to actually figure out velocity

    mp = 0
    for obstacle in obstacles:
        mp += CollisionController(obstacle, list_pos, list_vel)
    mp += AttractionController(goal, list_pos, list_vel)
    oldStates = list_pos
